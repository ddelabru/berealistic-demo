...
#
# Changes (August 2015 by Dominic Delabruere):
# Modified to load tilesets from relative filenames
...
class Tileset(object):
	...
	@classmethod
	def fromxml(cls, tag, filename, firstgid=None):
		...
		for c in tag.getchildren():
			if c.tag == "image":
				...
				source = os.path.join(
					source, c.attrib['source'])
				tileset.add_image(source)
			elif c.tag == 'tile':
